...
export function LogoCloud({ logos, className, ...props }: LogoCloudProps) {
  return (
    <div className={`relative mx-auto max-w-3xl bg-transparent py-6 md:border-x border-white/10 ${className}`} {...props}>
      ...
      <InfiniteSlider gap={42} reverse speed={60} speedOnHover={20}>
        {logos.map((logo) => (
          <img
            alt={logo.alt}
            className="pointer-events-none h-4 select-none md:h-5 invert"
            height="auto"
            key={`logo-${logo.alt}`}
            loading="lazy"
            src={logo.src}
